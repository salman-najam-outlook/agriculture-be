...
const { portugeseTranslation } = require('../cropReportings/translationKeys/portugese');
    ...
    for (const [index, { values, range }] of googlesheetData?.valueRanges.entries()) {
      ...
      for (row of values) {
        try {
          ...
          if (portugeseTranslation[moduleAttributeName.trim()]) {
            const moduleAttribute = await db.CropRecommendationModuleAttribute.findOne({
              raw: true,
              attributes: ['type', 'id'],
              where: { ddName: portugeseTranslation[moduleAttributeName.trim()] },
            });
            ...
            console.log('MISSING MODULE ATTRIBUTE TRANSLATION', moduleAttributeName.trim(), sheetName);
          }
        } catch (e) {
          console.log('error =>', e);
        }
