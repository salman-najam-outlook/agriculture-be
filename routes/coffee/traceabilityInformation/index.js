...
router.post("/", auth, async (req, res) => {
  try {
    ...
    const isExisting = await db.TraceabilityInformation.findOne({
      where: { coffee_plantation_id: coffee_plantation_id },
    });
    ...
    let traceabilityInfo = !isExisting
      ? await db.TraceabilityInformation.create(dataToSave)
      : await db.TraceabilityInformation.update(dataToSave, {
...
router.get("/", auth, async (req, res) => {
  try {
    ...
    let traceability = await db.TraceabilityInformation.findAll({
      ...
      // if(deepCopy) {
      //   deepCopy.farmerTraceabilityQRLink = `https://${process.env.TRACEABILITY_URL}/trace-your-product/#/farmProfile?ID=${userId}&currentEnv=${process.env.NODE_ENV}`
      // }
...
router.delete("/:id", auth, async function (req, res) {
  try {
    ...
    await db.TraceabilityInformation.destroy({
