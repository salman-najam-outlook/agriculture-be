...
router.get('/', auth, translation, async (req, res) => {
  try {
    ...
    let query = {
      ...
      order: orderBy,
      include: [
        { model: db.Option, as: 'fuel_type', attributes: ['id', 'name', 'recordId'] },
 ...
 *                email:
 *                  type: string
 *            example: {"farm":[1],"segment":[2],"displayName":"test","category":2,"activity":2,"group":{"id":1,"name":"Group1","recordId":"egr100"},"modeOfOperation":{"id":1,"name":"Mode of operation1","recordId":"emoo100"},"equipmentName":{"id":1,"name":"Equipment1","recordId":"ena100"},"identificationNumber":"58","serialNumber":343,"modelOrBrand":"gh200","yearOfManufacture":2012,"yearOfPurchase":2012,"quantity":2,"fuelType":{"id": -1, "name": "Petrol", "recordId": "eft100"},"energyConsumption":22,"loanStatus":"Cleared","isDefault":1,"equipmentType":"set"}
 *     responses:
 *        '200':
 *           description: Success
 *           content:
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (equipment?.category || equipment?.activity) {
      ...
      let equipmentName = await db.EquipmentName.findOne({ where: { recordId: equipment.equipmentName.recordId } });

      if (equipmentName) {
        await equipmentName.update({ name: equipment.equipmentName.name }, {
          transaction: t,
        });
      } else if (equipment.equipmentName.id === -1) {
        equipmentName = await db.EquipmentName.create({
          activity: equipment.activity,
          name: equipment.equipmentName.name,
          recordId: equipment.equipmentName.recordId,
          userID: req.user.id,
        }, {
          transaction: t,
        });
      ...
      // let equipmentFuelType = await db.Option.findOne({ where: { recordId: equipment.fuelType.recordId, groupName: 'fuel-type' } });
      
      // if (equipmentFuelType) {
      //   await equipmentFuelType.update({ name: equipment.fuelType.name }, {
      //     transaction: t,
      //   });
      // } else if (equipment.fuelType && equipment.fuelType.id === -1) {
      //   equipmentFuelType = await db.Option.create({
      //     name: equipment.fuelType.name,
      //     groupName: 'fuel-type',
      //     recordId: equipment.fuelType.recordId,
      //     userID: req.user.id,
      //   }, {
      //     transaction: t,
      //   });
      ...
      // let groupExists = await db.EquipmentGroup.findOne({
      //   where: {
      //     recordId: equipment.group.recordId,
      //   },
      // });

      // if (groupExists) {
      //   await groupExists.update({ name: equipment.group.name });
      // } else {
      //   groupExists = await db.EquipmentGroup.create({
      //     name: equipment.group.name,
      //     recordId: equipment.group.recordId,
      //     userID: equipment.userID,
      //   })
      // }

      let modeOfOperationExists = await db.EquipmentModeOfOperation.findOne({
        where: {
          recordId: equipment.modeOfOperation.recordId,
        },
      });

      if (modeOfOperationExists) {
        await modeOfOperationExists.update({ name: equipment.modeOfOperation.name }, {
          transaction: t,
        });
      } else {
        modeOfOperationExists = await db.EquipmentModeOfOperation.create({
          name: equipment.modeOfOperation.name,
          recordId: equipment.modeOfOperation.recordId,
          userID: equipment.userID
        }, {
          transaction: t,
      ...
      //  Equipment Data
      const equipmentData = await db.Equipment.create({
        ...equipment,
        // group: groupExists.id,
        // fuelType: equipmentFuelType.id,
        modeOfOperation: modeOfOperationExists.id,
...
router.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      //  Equipment Data
      const updatedEquipmentData = await db.Equipment.update(equipment, {
        where: {
          id: req.params.id,
        },
        transaction: t,
 ...
 *                   data:
 *                     type: object
 *                 example: { "success": true,"code": 200,"message": "Equipment is deleted successfully.","data": {"displayName": "testupd","category": 2,"activity": 2,"group": 3,"modeOfOperation": 2,"equipmentName": 2,"identificationNumber": "58","serialNumber": 343,"modelOrBrand": "gh200","yearOfManufacture": 2012,"yearOfPurchase": 2012,"quantity": 2,"fuelType": "Petrol","energyConsumption": 22,"loanStatus": "Cleared","isDefault": 1,"equipmentType": "set","userID": 171}}
 */
...
router.post('/fuelType', auth, translation, async (req, res) => {
  try {
    ...
    fuelTypeExists = await db.Option.findOne({
      where: {
        groupName: 'fuel-type',
        recordId: recordId,
      },
    ...
    if (fuelTypeExists === null) {
      const fuelType = await db.Option.create({
        name: name,
        groupName: 'fuel-type',
        recordId: recordId,
        userID: req.user.id,
      ...
      const fuelType = await fuelTypeExists.update({
        name,
      });
