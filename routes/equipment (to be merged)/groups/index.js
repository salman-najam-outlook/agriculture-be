 ...
 *                   data:
 *                     type: object
 *                 example: { "success": true, "code": 200, "message": "Fetched successfully.", "data": [ { "id": 1, "name": "Tractors Collection", recordId: "egr100" } ] }
 */
...
router.post(
  ...
  async (req, res) => {
    try {
      let equipmentGroupExists = null;
      
      if(req.body.recordId){
        equipmentGroupExists = await db.EquipmentGroup.findOne({
          where: {
      ...
      if (equipmentGroupExists === null) {
        ...
        const equipmentGroup = await equipmentGroupExists.update({
          name: req.body.name
        });
