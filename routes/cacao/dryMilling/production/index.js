...
router.post(
    ...
    async (req, res) => {
      try {
        ...
        const production = await db.cacaoDryMillingProduction.create(set);
  ...
  router.get(
    ...
    async (req, res) => {
      try {
        ...
        let { count: totalRows, rows } =
          await db.cacaoDryMillingProduction.findAndCountAll({
            where,
            offset: (page - 1) * limit,
            limit: limit,
            order: [[col, order]],
   ...
   router.put('/target/:id', auth, validatorProduction.editProductionTarget(), validationErrorHandler, async (req, res) => {
    try {  
        ...
        const targetRecord = await db.cacaoDryMillingProduction.findOne({
            where: {
            id, buyingStationId
            }
        })
        ...
        try {
            // Updating the Target
            await db.cacaoDryMillingProduction.update(set, {
                where: {
                    id
                }
            }, { transaction });
