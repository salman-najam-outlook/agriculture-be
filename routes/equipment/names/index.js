...
const validationErrorHandler = require(rootPath +
  ...
  const {
    addEquipmentActivityToEquipmentName
  } = require('../utils');
/**
 * @swagger
 * /equipments/names:
 ...
 *                   data:
 *                     type: object
 *                 example: { "success": true, "code": 200, "message": "Fetched successfully.", "data": [ { "id": 3, "name": "Hammer", "equipment_name_activity": { "id": 1, "name": "General", "category": 1, "createdAt": "2021-12-06T16:40:31.000Z", "updatedAt": "2021-12-06T16:40:31.000Z" } } ] }
 */
    ...
    try {
      ...
      if (equipmentNameExists === null) {
        ...
        try {
          ...
          if(req.body.activityIds?.length){
            await addEquipmentActivityToEquipmentName(req.body.activityIds, equipmentName.dataValues, t);
          }
          ...
          return res.json(
            errorRespSync({
              ...
              msg: err.msg ?? "Failed to save equipment name, please retry.",
            })
          );
        }
      } else {
        return res.status(error.code.CONFLICT).json(
          errorRespSync({
            msg: error.code.UNPROCESSABLE_ENTITY,
            code: 409,
            data: equipmentNameExists,
          })
        );
      }
    } catch (err) {
