...
exports.validateEquipmentPayloadDataExistsInDB = async (equipment, res) => {
  ...
  if (equipment?.modeOfOperation) {
    ...
    const defaultEquipRes = await db.EquipmentModeOfOperation.findOne({
      where: {
        id: equipment.modeOfOperation,
      }
    })
    let isDefaultEquip = defaultModeOfOperation.includes(defaultEquipRes.dataValues.name)

    if (modeOfOperationExists === null && isDefaultEquip === null) {
      throw {
...
exports.addEquipmentActivityToEquipment = async (activityIds, equipmentInstance, transaction) => {
  await equipmentInstance.setEquipment_equipment_activity(activityIds, {transaction: transaction})
}

exports.addEquipmentActivityToEquipmentName = async (activityIds, equipmentNameInstance, transaction) => {
  await Promise.all(
    activityIds.map(async (activityId) => {
      await db.EquipmentNameEquipmentActivity.create(
        {
          equipmentNameId: equipmentNameInstance.id,
          equipmentActivityId: activityId
        },
        {
          transaction
        }
...
exports.updateEquipmentActivityToEquipment = async (activityIds, equipmentInstance, transaction) => {
  await equipmentInstance.setEquipment_equipment_activity(activityIds, { transaction: transaction })
}
