...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const traceability = await db.Traceability.create(dataToSave);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const traceability = await db.Traceability.findOne({
      ...
      let updatedData = await db.Traceability.update(set, {
        where: { id },
      });
...
router.get(
  ...
  async (req, res) => {
    ...
    const { organization, subOrgId } = req.user;
      ...
      let traceability = await db.Traceability.findAll({
      include: [
        {
          attributes: [],
          model: db.user,
