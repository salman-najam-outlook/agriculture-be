...
const { deleteFileS3, getSignedURL } = require(rootPath + '/helpers/aws_s3');
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const label = await db.CropsTraceabilityLabels.create(set);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let label = await db.CropsTraceabilityLabels.findAll({
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const existingLabel = await db.CropsTraceabilityLabels.findOne({
        attributes: ['logo'],
        where: { id }
      });
      ...
      let updatedData = await db.CropsTraceabilityLabels.update(set, {
        where: { id },
      });
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let label = []
      label = await db.CropLabelFormat.findAll();
      let promiseArr = []
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let label = await db.CropsTraceabilityLabels.findOne({
        where: { id: id },
      });
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let label = await db.CropLabelFormat.findOne({
        where: { id: id },
      });
...
router.delete(
  ...
  async function (req, res) {
    try {
      ...
      await db.CropsTraceabilityLabels.destroy({
