...
router.post(
  ...
  async (req, res, next) => {
    ...
    fileUpload(req, res, function (err) {
      if (err instanceof multer.MulterError || err) {
        return res.status(400).json(errorRespSync({ code: 400, msg: err.message }));
      } else if (err) {
        return res.status(500).json(errorRespSync({ code: 500, msg: "File upload error" }));
      } else {
  ...
  async (req, res) => {
    try {
      ...
      const traceability = await db.CropsTraceability.create(dataToSave);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const traceability = await db.CropsTraceability.findOne({
      ...
      let updatedData = await db.CropsTraceability.update(set, {
        where: { id },
      });
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let traceability = await db.CropsTraceability.findAll({
...
router.delete(
  ...
  async function (req, res) {
    try {
      ...
      await db.CropsTraceability.destroy({
