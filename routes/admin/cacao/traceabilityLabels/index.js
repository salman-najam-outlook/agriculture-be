...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const label = await db.CacaoTraceabilityLabels.create(set);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let label = await db.CacaoTraceabilityLabels.findAll({
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const labelFormat = await db.CacaoLabelFormat.create(set);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const existingLabel = await db.CacaoTraceabilityLabels.findOne({
        attributes: ['logo'],
        where: { id }
      });
      ...
      let updatedData = await db.CacaoTraceabilityLabels.update(set, {
        where: { id },
      });
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let label = []
      label = await db.CacaoLabelFormat.findAll();
      let promiseArr = []
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let label = await db.CacaoTraceabilityLabels.findOne({
        where: { id: id },
      });
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      let label = await db.CacaoLabelFormat.findOne({
        where: { id: id },
      });
...
router.delete(
  ...
  async function (req, res) {
    try {
      ...
      await db.CacaoTraceabilityLabels.destroy({
        where: { id: id, user_id: userId },
      });

      return res.json(
        successRespSync({
          msg: 'Cacao Tracebility Label deleted.',
        })
      );
