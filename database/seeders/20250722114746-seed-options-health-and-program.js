...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    ...
    console.log(`🌱 Starting seeder for ${optionsToSeed.length} options...`);

    // Check which options already exist by ID (any ID in the options table)
    ...
    // Check if any of our target IDs already exist with different names/groups (ID conflicts)
    const conflictingIds = await queryInterface.sequelize.query(
      `SELECT id, name, groupName FROM options WHERE id IN (${optionsToSeed.map(opt => opt.id).join(',')})`,
      { type: Sequelize.QueryTypes.SELECT }
    );

    console.log( 'conflictingIds-->', conflictingIds);
  ...
  down: async (queryInterface, Sequelize) => {
    ...
    console.log(`🗑️  Starting rollback for seeder...`);
