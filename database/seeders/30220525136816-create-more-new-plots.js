...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    let idGen = 2400;
    const requests = farmNames.map((farmName) => {
    ...
    const plots = [
      ...
      const {farm_id, ...plotData} = plot;

      if (currentFarm === null || currentFarm != farm_id) {
        request_id = `RE-JOSE${idGen}`;
        idGen = idGen + 1;
        currentFarm = farm_id;
      }

      if (!request_id) {
        throw Error("ID Generation Error!")
      }
